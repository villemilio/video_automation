...
def fetch_short_story():
    """
    Fetches a simulated Reddit-style question and short story using OpenAI's API.
    Returns a tuple (title, text), where:
      - 'title' is the Reddit-style question
      - 'text' is the full story formatted as a response
    """
    try:
        # Call OpenAI Chat API using the updated OpenAI Client
        ...
        # Parse out the question/story
        question_marker = "Question:"
        story_marker = "Story:"
        if question_marker in full_text and story_marker in full_text:
            q_start = full_text.index(question_marker) + len(question_marker)
            s_start = full_text.index(story_marker)
            question = full_text[q_start:s_start].strip()
            story = full_text[s_start + len(story_marker):].strip()
        else:
            # Fallback if formatting is incorrect
            question = "Tell me about a time when something interesting happened."
            story = full_text
...
def main():
    #  Fetch a random story (now generated by OpenAI instead of Reddit)
    story_data = fetch_short_story()
    if not story_data:
        print("No valid stories found. Exiting.")
        return
